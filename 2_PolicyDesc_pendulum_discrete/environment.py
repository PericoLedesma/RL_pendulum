        ...
        try:
            for eps in range(n_episodes):
                ...
                while not truncated or not done:
                    act_index = agent.get_action(state)

                    state_, reward, terminated, truncated, info = self.env.step([agent.action_space[act_index].item()])

                    state_evolution.append((eps, step, state))
                    eps_data.append((eps, state, act_index, reward))
                    action_history.append(agent.action_space[act_index].item())
